...
void printTree(size_t level, const char * indexName){
  ...
  printRec(f,node,0,node.left,0,max);
  printRec(f,node,1,node.right,0,max);
...
void printRec(FILE *f,Node node,int flag,int type,int count,int level){
  ...
  if(!f||type==-1||count==level) return;

  /*flag=0 si es nodo izquierdo*/
  if(flag==0){
    ...
    /*type=id, obtenido del anterior node.left*/
    fprintf(stdout,"%sl %s (%d): %d",cad,node.book_id,type,node.offset);
    fprintf(stdout,"\n");

  }

  /*flag=1 si es nodo derecho*/
  if(flag==1){
    ...
    fprintf(stdout,"%sr %s (%d): %d",cad,node.book_id,type,node.offset);
    fprintf(stdout,"\n");
  }

  count++;

  printRec(f,node,0,node.left,count,level);
  printRec(f,node,1,node.right,count,level);
